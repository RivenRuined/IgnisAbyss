...
/* 
  ...
  1) D-Pad is now placed in its own row (Row 6), centered below the meters.
  2) Added a new "Movement" numeric input (for touch speed only) alongside "Speed".
  3) Renamed "Aggression" to "Agro".
  4) Ensured buttons and number inputs are touch-responsive by removing extra z-index
     settings that were causing overlap issues.
...
// "Agro" replaces "Aggression"; "Movement" controls touch speed only
...
function setup() {
  ...
  // Keep controlPanel in normal flow (relative positioning)
  ...
  speedInput = createInput('1.95', 'number'); // For keyboard movement speed
  speedInput.parent(row2);
  speedInput.style("font-size", "18px");
  speedInput.style("width", "60px");
  speedInput.style("text-align", "center");

  movementInput = createInput('1.0', 'number'); // For touch movement speed
  ...
  // Note: Removed high z-index so row6 does not overlap previous rows
...
function draw() {
  ...
  // D-Pad continuous movement for touch controls (uses "Movement" input)
...
function touchStarted() {
  if (touches.length > 0) {
    let t = touches[0];
    // If touch begins in left 30% of canvas, consider d-pad activation
    if (t.x < width * 0.3) {
      dPadActive = true;
    } else {
      touchStartX = t.x;
...
function touchMoved() {
  if (touches.length > 0) {
    let t = touches[0];
    if (dPadActive) {
      let dx = t.x - 80; // dPadCenterX
      let dy = t.y - 820; // dPadCenterY
      // Use the movement slider value for touch
...
function touchMoved() {
  if (touches.length > 0) {
    let t = touches[0];
    if (dPadActive) {
      let dx = t.x - 80; // dPadCenterX = 80
      let dy = t.y - 820; // dPadCenterY = 820
      let factor = parseFloat(movementInput.value());
      singularity.pos.x += dx * factor;
      singularity.pos.y += dy * factor;
    } else {
...
function createDPad() {
  ...
  dPadContainer.parent(controlPanel);
  // Place D-Pad in its own row (Row 6) without high z-index so it doesn't cover other elements
  ...
  dPadContainer.style("margin-bottom", "10px");
...
function createDPad() {
  ...
  dPadContainer.style("gap", "5px");
  dPadContainer.style("margin-bottom", "10px");
  // No high z-index here so it doesn't cover other elements
...
function draw() {
  ...
  // D-Pad continuous movement for touch (using Movement input)
  if (dPadDirection.x !== 0 || dPadDirection.y !== 0) {
    let touchSpeed = parseFloat(movementInput.value());
    singularity.pos.x += dPadDirection.x * touchSpeed;
    singularity.pos.y += dPadDirection.y * touchSpeed;
